...
const emails = {
  ...
  manager:  `rbac-mgr-${ts}@test.example`,
...
async function setupOrg() {
  ...
  for (const role of ['manager', 'engineer', 'operator', 'viewer']) {
...
const MATRIX = [
  {
    name: 'create project',
    allowed: ['admin', 'manager', 'engineer'],
  ...
  {
    name: 'run simulation',
    allowed: ['admin', 'manager', 'engineer', 'operator'],
  ...
  {
    name: 'list projects',
    allowed: ['admin', 'manager', 'engineer', 'operator', 'viewer'],
...
describe('RBAC role × endpoint matrix', () => {
  if (SKIP) {
    ...
    describe('last-admin protection', () => {
      it('a deactivated admin cannot demote or delete the last active admin, even on a valid JWT', async () => {
        // Invite a second admin B, log them in, then deactivate B. B's JWT is
        // still valid. Before Phase 0 the request reached the controller and
        // the last-admin guard refused it with 409. Now the per-request role
        // check (middleware/auth.js) sees the row is inactive and refuses at
        // the door with 401 — the stronger guarantee: a deactivated account
        // does nothing at all, not just nothing dangerous. The 409 guard stays
        // in the controller as the fallback for the fail-open window when the
        // role lookup itself is unavailable.
        ...
        // B (stale but valid JWT) tries to demote A — refused before the route
        const demote = await authed('patch', `/api/v1/admin/members/${userA.id}`, tokenB)
          .send({ role: 'engineer' });
        expect(demote.status).toBe(401);
        expect(demote.body.error).toMatch(/deactivated/i);

        // ... and to delete A — also refused
        const del = await authed('delete', `/api/v1/admin/members/${userA.id}`, tokenB);
        expect(del.status).toBe(401);

        // A is untouched and cannot remove themself either (self guard, 403)
        const self = await authed('patch', `/api/v1/admin/members/${userA.id}`, tokens.admin)
          .send({ role: 'engineer' });
        expect(self.status).toBe(403);
        const after = await authed('get', '/api/v1/admin/members', tokens.admin);
        const a = after.body.find((m) => m.email === emails.admin);
