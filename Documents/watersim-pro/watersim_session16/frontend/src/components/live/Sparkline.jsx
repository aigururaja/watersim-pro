  ...
  return (
    <div className="flex items-center gap-3" data-tag={tag}>
      <div className="min-w-0 w-32">
        <div className="font-mono text-[11px] text-ink truncate">{tag}</div>
        <div className="text-[10px] text-ink-3 truncate" title={name}>{name}</div>
      </div>
      ...
      <div className="font-mono text-sm text-ink tabular-nums w-16 text-right">{fmtValue(last)}<span className="text-[10px] text-ink-3 ml-0.5">{unit || ''}</span></div>
    </div>
  );
}
