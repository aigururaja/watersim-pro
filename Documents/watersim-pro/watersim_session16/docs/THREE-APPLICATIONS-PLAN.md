# WaterSim Pro — the three applications: gap map and build plan
...
| Shared asset identity | **partial** | Both sides have a natural key — the CMMS's `asset_code`, this side's ISA tag. Map them 1:1 (`RFP-P-201` *is* the asset code) and carry the WaterSim tag id in the CMMS's existing `custom_fields`, so neither schema changes. | S | tag registry (Phase 0), `assets.custom_fields` |
...
- **Asset identity = ISA tag = `asset_code`.** One row in the CMMS's `assets` per equipment tag (`RFP-P-201`), sub-assets for units (`/1`, `/2`); the WaterSim `tag_id` goes in `assets.custom_fields.watersim_tag_id`. A one-off sync script seeds the CMMS from the tag registry — its lookup tables (type, make, model, vendor) are exactly the proposal's equipment schedule (Johnson, Grundfos, Beta, Milton Roy, Hiller).
...
2. **WhatsApp provider.** Twilio WhatsApp API is fastest to integrate; Meta Cloud API is cheaper at volume. Both need Business verification and pre-approved templates — **weeks of lead time; start now.**
