...
The Android app is a **Trusted Web Activity**: a thin signed shell that opens
`https://${DOMAIN}/dashboard` full screen. There is no second codebase; every
frontend deploy updates what the app shows. It needs three things from the
site, all shipped in `frontend/public` by the normal build:

- `manifest.json` and `icons/` (the installable-app manifest and icons; the
  name is `.json` because this host's nginx has no MIME type for
  `.webmanifest`), and `sw.js` with `offline.html` (a worker that handles page
  navigations only, so it can never serve stale plant data);
- `.well-known/assetlinks.json`, which names the package
  `com.inferconautomation.safekrit` and the SHA-256 of the signing
  certificate. Android checks it on first launch; without it the app still
  works but shows a browser bar. Verify with
...
- `downloads/safekrit.apk`, the signed APK the landing page links to.

The wrapper is generated by Bubblewrap from `android/twa-manifest.json` (the
only tracked file under `android/`). To rebuild on the build PC, with JDK 17
first on `PATH` and the signing key's password in the two environment
variables Bubblewrap reads (`BUBBLEWRAP_KEYSTORE_PASSWORD`,
`BUBBLEWRAP_KEY_PASSWORD`):

```bash
cd android
npx @bubblewrap/cli update --skipVersionUpgrade    # regenerate the Gradle project from twa-manifest.json
npx @bubblewrap/cli build --skipPwaValidation      # app-release-signed.apk + app-release-bundle.aab
cp app-release-signed.apk ../frontend/public/downloads/safekrit.apk
```

Raise `appVersionCode` (and `appVersion`) in `twa-manifest.json` for every
release people will install over an older one. The signing key lives outside
the repository and must be backed up: an app signed with a different key
cannot update the installed one, and `assetlinks.json` must carry the new
fingerprint. For Google Play, upload `app-release-bundle.aab`; after the
first upload add Play's own app-signing fingerprint to `assetlinks.json` as a
second entry. A different `${DOMAIN}` needs its own `host`, icon URLs and
`fullScopeUrl` in `twa-manifest.json` and a rebuild.
